// ==========================================
// BLOCK: CACHE-PROOF AUTOMATED FOOTER (V4.6 - INSTANT OVERRIDE)
// ==========================================
document.addEventListener("DOMContentLoaded", function() {
    const footerSlot = document.getElementById("footer-component");
    if (!footerSlot) return;

    // Hardcoded fallback data jo direct dikhega agar response delay ho
    const fallbackEmail = "genzest3018@gmail.com";
    const fallbackCopyright = "&copy; 2026 GENZEST Lab. Built with clean sheets & 0% manual database maintenance.";

    // 1. Initial Render (Instant Build)
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-8 pb-8 border-b theme-border">
                ...
                <div>
                    ...
                    <p class="text-xs theme-muted leading-relaxed">Sponsorships & Queries:<br>
                        <span id="dyn-contact-email" class="theme-text font-medium text-white">${fallbackEmail}</span>
                    </p>
                </div>
            </div>
    ...
    // 2. FORCE SYNC ENGINE: Forcefully reads from layout configs sheet
    async function forceSheetSync() {
        if (typeof getLiveLayoutConfigs !== "function") {
            setTimeout(forceSheetSync, 400);
            return;
        }

        try {
            // Adds a timestamp parameter to drop old memory leak cache data
            ...
            if (layoutConfigs && layoutConfigs.length > 0) {
                ...
                if (emailConfig && emailConfig.value && emailEl) {
                    emailEl.innerText = emailConfig.value.trim();
                }
                // DIRECT DOM OVERRIDE: Sheet ka text bina kisi loop delay ke seedha overwrite hoga
                if (copyrightConfig && copyrightConfig.value && copyrightEl) {
                    copyrightEl.innerHTML = copyrightConfig.value.trim();
                }
            }
        } catch (error) {
            console.log("Background override bypassed, local backup operational.");
        }
    }
